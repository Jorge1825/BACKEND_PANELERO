...
farmCtrl.getFarms = async (req, res) => {
  ...
  //buscamos todas las fincas que esten en el arreglo de farms del usuario
    for (let i = 0; i < decoded.farms.length; i++) {
      const farm = await Farm.findById(decoded.farms[i]._id);
      farms.push(farm);
    } 
    //eliminar los null del arreglo de farms
    farms = farms.filter((farm) => farm !== null);
