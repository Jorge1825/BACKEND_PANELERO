...
adminExpensesCtrl.getAdminExpenses = async (req,res) => {
  ...
  try {
    const adminExpenses = await AdminExpenses.find({farm}).populate("spent")
...
adminExpensesCtrl.adminExpensesId = async (req, res) => {
  const { id } = req.params;
  try {
    const adminExpensesId = await AdminExpenses.findById(id).populate("spent")
...
adminExpensesCtrl.registerAdminExpenses = async (req, res) => {
  const { farm } = req.headers;
  const {name, description, worth,spent} = req.body;
  try {
