...
activityExpensesCtrl.getActivityExpenses = async (req, res) => {
  ...
  try {
    const activityExpenses = await ActivityExpenses.find({ farm }).populate("work").populate("spent");
...
activityExpensesCtrl.activityExpensesId = async (req, res) => {
  const { id } = req.params;
  try {
    const activityExpenses = await ActivityExpenses.findById(id).populate("work").populate("spent")
...
activityExpensesCtrl.registerActivityExpenses = async (req, res) => {
  const { name, description, worth,work,spent } = req.body;
  const { farm } = req.headers;
  try {
